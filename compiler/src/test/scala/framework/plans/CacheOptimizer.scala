...
class TestCacheOptimizer extends FunSuite with MaterializeNRC with NRCTranslator {
  ...
  test("input hash"){
    ...
    val p1 = CacheOptimizer.signature(cust1)
    val p2 = CacheOptimizer.signature(cust2)
    assert(p1 != p2)
  ...
  test("project hash"){
    ...
    val p1 = CacheOptimizer.signature(custPlan1)
    val p2 = CacheOptimizer.signature(custPlan2)
    assert(p1 == p2)
    ...
    val p3 = CacheOptimizer.signature(ordPlan)
    assert(p1 != p3)
  ...
  test("join hash"){
    ...
    val p1 = CacheOptimizer.signature(joinPlan1)
    val p2 = CacheOptimizer.signature(joinPlan2)
    assert(p1 == p2)
    ...
    val p3 = CacheOptimizer.signature(joinPlan3)
    assert(p2 != p3)
  ...
  test("unnest hash"){
    ...
    val p1 = CacheOptimizer.signature(unnestPlan1)
    val p2 = CacheOptimizer.signature(unnestPlan2)
    assert(p1 == p2)
  ...
  test("reduce hash"){
    ...
    val p1 = CacheOptimizer.signature(reducePlan1)
    val p2 = CacheOptimizer.signature(reducePlan2)
    assert(p1 == p2)
  ...
  test("input SEs"){
    ...
    val p1 = CacheOptimizer.subexpressions(cust1)
    val p2 = CacheOptimizer.subexpressions(cust2)
    assert(p1.size == 1)
    assert(p2.size == 1)
    assert(p1 != p2)
  ...
  test("select SEs"){
    ...
    val p2 = CacheOptimizer.subexpressions(cust2)
    val p3 = CacheOptimizer.subexpressions(cust3)
    ...
    val subs = CacheOptimizer.sharedSubs(Vector(cust1, cust2, cust3))
  ...
  test("join SEs"){
    ...
    val joinQuery1 = parser.parse(
      ...
      """, parser.term).get
    val joinPlan1 = getPlan(joinQuery1.asInstanceOf[Expr]).asInstanceOf[CExpr]
    val p3 = CacheOptimizer.subexpressions(joinPlan1)

    assert((p1.values.toSet & p3.values.toSet).nonEmpty)

    val subs = CacheOptimizer.sharedSubs(Vector(cust1, cust2, joinPlan1))
    // input, index, selects
    assert(subs.size == 3)
    for (k <- p2.values){
  ...
  test("reduce SEs"){
    ...
    val subs = CacheOptimizer.sharedSubs(Vector(reducePlan1, reducePlan2))
    assert(subs.size == 8)
