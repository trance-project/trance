...
class TestGreedyCachePlanner extends FunSuite with MaterializeNRC with NRCTranslator {
  ...
  test("Expression below reduce shared more"){
    ...
    val plan0 = getPlan(query0.asInstanceOf[Program])
    ...
    val progs = Vector(plan0, plan1, plan2, plan3).zipWithIndex
    val plans = progs.flatMap{ case (prog, id) => 
      prog.exprs.map(e => e match {
        case CNamed(name, p) => (p, id)
        case _ => (e, id)
