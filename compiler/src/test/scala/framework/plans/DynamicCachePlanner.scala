...
class TestDynamicCachePlanner extends FunSuite with MaterializeNRC with NRCTranslator {
  ...
  test("aggregates"){
    ...
    // result0.selected.foreach(s => println(Printer.quote(s._2)))
    ...
    // result1.selected.foreach(s => println(Printer.quote(s._2)))
