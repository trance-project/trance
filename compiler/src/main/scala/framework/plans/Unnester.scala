...
object Unnester {
  ...
  type Ctx = (Map[String, Type], Map[String, Type], Option[CExpr], String, Int)
  ...
  @inline def level(implicit ctx: Ctx): Int = ctx._5
  ...
  def unnest(e: CExpr)(implicit ctx: Ctx): CExpr = e match {
    ...
    case Comprehension(e1, v, p, e2) if u.isEmpty && w.isEmpty && E.isEmpty =>
      ...
      val nE = unnest(e1)((u, w, E, tag, level))
      ...
      unnest(e2)((u, nv.tp.attrs, Some(Select(ne1, nv, replace(p, nv), level)), tag, level))
    ...
    case c @ Comprehension(e1 @ Project(e0, f), v, p, e2) if !w.isEmpty =>
      ...
      // u is empty C7
      if (u.isEmpty){
        val nE = Unnest(E.get, wvar(w), f, v, p, Nil, level)
        unnest(e2)((u, flat((w - f), v.tp), Some(nE), tag, level))
      // u is nonEmpty C10
      }else{
        ...
        unnest(e2)((u - f), flat(nw - f, v.tp.outer), Some(nE), tag, level)
      }
    ...
    case e1 @ CLookup(lbl, dict) => 
      ...
      unnest(Comprehension(e1, nv, Constant(true), Sng(nv)))((u, w, E, tag, level))
    ...
    case Comprehension(CLookup(lbl, e1), v1, filt, e2) => 
      ...
      val dict = unnest(e1)((u, w, E, tag, level))
      val nv = wvar(w)
    ...
    case Comprehension(e1:Comprehension, v, p, Constant(c)) => unnest(e1)((u, w, E, tag, level)) match {
    ...
    case CReduceBy(e1, v1, keys, values) => unnest(e1)((u, w, E, tag, level)) match {
      ...
      case e2 => E match {
        case Some(e3) => 
          ...
          Reduce(e2, v1, nkeys :+ "_1", values, level)
        case _ => Reduce(e2, v1, keys, values, level)
      }
    }
    ...
    case Comprehension(e1:CReduceBy, v, cond, e2) if !w.isEmpty => 
      val nE = unnest(e1)((w, w, E, tag, level)) match {
        case Nest(e2:Reduce, _, _, _, _, _, _, _) => RemoveNulls(e2)
        case r:Reduce =>  RemoveNulls(r)
        case _ => ???
      }
      val nw = flat(w, nE.tp.asInstanceOf[BagCType].tp)
      unnest(e2)((u, nw, Some(nE), tag, level))

    // same as the sumBy handling, except catching dedup
    case CDeDup(e1, _) => unnest(e1)((u, w, E, tag, level)) match {
      // if it came from above then 
      case Nest(e2, v2, keys, values, filter, nulls, ctag, l) =>
        val indices = u.keySet.filter(x => x.contains("_index"))
        val keys2 = (indices ++ keys).toList
        CDeDup(Projection(e2, v2, replace(extendIf(keys2, values, v2), v2), Nil, l), l)
      case s =>  CDeDup(s, level)
    }

    // Dedup generator
    // handles the same as the above case
    case Comprehension(e1:CDeDup, v, cond, e2) if !w.isEmpty =>
      val nE = RemoveNulls(unnest(e1)((w, w, E, tag, level)))
      val nw = flat(w, nE.tp.asInstanceOf[BagCType].tp)
      unnest(liftFilter(cond, e2))((u, nw, Some(nE), tag, level))
    ...
    case Comprehension(e1, v, cond, e2) if !w.isEmpty =>
      assert(!E.isEmpty)
      val name = getName(e1)
      val right = AddIndex(unnest(e1)((u, w, E, tag, level)), name+"_index")
      val nv = Variable(v.name, right.tp.tp)

      // if u is empty generate inner join, if not then outer
	    val (nw, nE) = 
        if (u.isEmpty) (flat(w, nv.tp), Join(E.get, wvar(w), Select(right, nv, Constant(true), level), nv, cond, Nil, level))
        else (flat(w, nv.tp.outer), OuterJoin(E.get, wvar(w), Select(right, nv, Constant(true), level), nv, cond, Nil, level))
      unnest(e2)((u, nw, Some(nE), tag, level))

    // translate a groupBy directly into a Gamma
    case CGroupBy(e1, v1, keys, values, gname) => 
      val nE = unnest(e1)((u, w, E, gname, level)) 
      val nv = wvar(w)
      val tup = Record(values.map(v => v -> Project(nv, v)).toMap)
      Nest(nE, nv, keys, tup, Constant(true), (w.keySet -- u.keySet).toList, gname, level)
    ...
    case s @ If(cnd, Sng(t @ Record(fs)), nextIf) =>
      handleLevel(fs, x => If(cnd, Sng(x), nextIf))((u, w, E, tag, level))

    case s @ Sng(t @ Record(fs)) if !w.isEmpty =>
      handleLevel(fs, identity)((u, w, E, tag, level))
    ...
    case Bind(x, e1, e2) => 
      Bind(x, unnest(e1)((u, w, E, tag, level)), unnest(e2)((u, w, E, tag, level)))
    case FlatDict(e1) => FlatDict(unnest(e1)((u, w, E, tag, level)))
    case GroupDict(e1) => GroupDict(unnest(e1)((u, w, E, tag, level)))
    case LinearCSet(exprs) => LinearCSet(exprs.map(unnest(_)((u, w, E, tag, level))))
    case CNamed(n, exp) => 
      CNamed(n, unnest(exp)((u, w, E, tag, level)))
    case _ => e
