...
object Test1Agg1S extends TPCHBase {
  ...
  val (orders, orderRef) = varset(Test1Full.name, "c", Test1Full.program(Test1Full.name).varRef.asInstanceOf[BagExpr])
  val (parts, partRef) = varset("parts", "l", BagProject(orderRef, "o_parts"))
  ...
  val query = ReduceByKey(
    ForeachUnion(s1Ref, s1, 
      ForeachUnion(subTotals, BagProject(s1Ref, "o_parts"),
