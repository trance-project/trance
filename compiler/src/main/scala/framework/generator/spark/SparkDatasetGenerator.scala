...
class SparkDatasetGenerator(cache: Boolean, evaluate: Boolean, skew: Boolean = false, optLevel: Int = 2,
  ...
  def generate(e: CExpr): String = e match {
    ...
    case CDeDup(e1, l) => s"${generate(e1)}.distinct"
    ...
    case ej @ OuterJoin(left, v1, right, v2, Equals(Project(_, p1), Project(_, p2 @ "_1")), filt, l) if right.tp.isDict =>
    ...
    case er @ Reduce(in, v, key, value, l) =>
      handleType(er.tp.tp)
      val intp = generateType(in.tp.asInstanceOf[BagCType].tp)
