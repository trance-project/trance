...
case class Record2537(p_name: String, uniqueId: Long) extends CaseClassRecord
case class Record2539(n_name: String, part_names: List[Record2537], uniqueId: Long) extends CaseClassRecord
...
val S = TPCHLoader.loadSupplier[Supplier].toList
    val Query7 = { val x2497 = P.flatMap(x2460 => { 
...
x2497  }
    ...
    def f(){
      val x2533 = N.flatMap(x2510 => { 
    val x2511 = x2510.n_name 
    val x2530 = Query7.flatMap(x2512 => { 
        val x2513 = x2512.suppliers 
        val x2529 = x2513.flatMap(x2514 => { 
          if({val x2515 = x2514.s_nationkey 
          val x2516 = x2510.n_nationkey 
          val x2517 = x2515 == x2516 
          val x2518 = x2512.customers 
          val x2523 = x2518.foldLeft(0)((acc2538, x2519) => 
            if({val x2520 = x2519.c_nationkey 
            val x2521 = x2510.n_nationkey 
            val x2522 = x2520 == x2521 
            x2522   }) {  acc2538 + {1}} else {  acc2538}) 
          val x2524 = x2523 == 0 
          val x2525 = x2517 && x2524 
          x2525       }) {  val x2526 = x2512.p_name 
            val x2527 = Record2537(x2526, newId) 
            val x2528 = List(x2527) 
            x2528} else {  Nil}}) 
        x2529}) 
    val x2531 = Record2539(x2511, x2530, newId) 
    val x2532 = List(x2531) 
    x2532}) 
x2533 
    }
    var time = List[Long]()
