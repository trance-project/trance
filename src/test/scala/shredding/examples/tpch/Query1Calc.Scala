...
import shredding.examples.tpch._
    case class Record36(p_name: String, l_qty: Double, uniqueId: Long) extends CaseClassRecord
case class Record37(o_orderdate: String, o_parts: List[Record36], uniqueId: Long) extends CaseClassRecord
case class Query1Out(c_name: String, c_orders: List[Record37], uniqueId: Long) extends CaseClassRecord
object Query1Calc {
        ...
        val x25 = L.flatMap(x12 => { 
          ...
          x15   }) {  val x24 = P.flatMap(x16 => { 
              ...
              x19   }) {  val x20 = x16.p_name 
                ...
                val x22 = Record36(x20, x21, newId) 
                val x23 = List(x22) 
                x23} else {  Nil}}) 
            x24} else {  Nil}}) 
        val x26 = Record37(x11, x25, newId) 
        val x27 = List(x26) 
        x27} else {  Nil}}) 
    val x29 = Query1Out(x6, x28, newId) 
    val x30 = List(x29) 
    x30}) 
x31 
    }
