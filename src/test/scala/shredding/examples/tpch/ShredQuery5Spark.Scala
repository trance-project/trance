...
case class Record1351(p_name: String, cnt: Int)
case class Record1352(_1: Record1251, _2: (Iterable[Record1351]))
...
Query5__D_2customers_1.count
   ...
   def f() {
     val x1279 = () 
val x1280 = Record1251(x1279) 
val x1281 = List(x1280) 
val M_ctx1 = x1281
val x1284 = M_ctx1 
val x1285 = Query5__D_1 
val x1287 = x1285 
val x1290 = x1287.map{ case c => (x1284.head, c) } 
val x1292 = Query5__D_2customers_1 
val x1295 = x1292 
val x1300 = { val out1 = x1290.map{ case (a, null) => (null, (a, null)); case (x1296, x1297) => ({val x1299 = x1297.customers 
x1299}, (x1296, x1297)) }
  val out2 = x1295.flatMap(x1298 => x1298._2.map{case v2 => (x1298._1.lbl, v2)})
  out1.outerLookup(out2)
} 
val x1301 = Query5__D_2suppliers_1 
val x1304 = x1301 
val x1310 = { val out1 = x1300.map{ case (a, null) => (null, (a, null)); case ((x1305, x1306), x1307) => ({val x1309 = x1306.suppliers 
x1309}, ((x1305, x1306), x1307)) }
  val out2 = x1304.flatMap(x1308 => x1308._2.map{case v2 => (x1308._1.lbl, v2)})
  out1.outerLookup(out2)
} 
val x1322 = x1310.flatMap{ case (((x1311, x1312), x1313), x1314) => val x1321 = (x1314) 
x1321 match {
   case (null) => Nil
   case x1320 => List(({val x1315 = (x1311,x1312,x1313) 
x1315}, {val x1316 = x1313.c_nationkey 
val x1317 = x1314.s_nationkey 
...
val x1331 = x1322.flatMap{ case ((x1323, x1324, x1325), x1326) => val x1330 = (x1325) 
x1330 match {
   case x1328 if {val x1329 = x1326 == 0 
x1329} => List(({val x1327 = (x1323,x1324) 
x1327}, {1}))
   case x1328 => List(({val x1327 = (x1323,x1324) 
x1327}, 0))
 }
}.reduceByKey(_ + _) 
val x1340 = x1331.flatMap{ case ((x1332, x1333), x1334) => val x1339 = (x1333,x1334) 
x1339 match {
   case (_,0) => Nil 
   case x1338 => List(({val x1335 = (x1332) 
x1335}, {val x1336 = x1333.p_name 
val x1337 = Record1351(x1336, x1334) 
x1337}))
 }
}.groupByLabel() 
val x1345 = x1340.map{ case (x1341, x1342) => 
   val x1343 = (x1342) 
val x1344 = Record1352(x1341, x1343) 
x1344 
} 
val M_flat1 = x1345
//println("M_flat1")
...
val res = x1346.count
   ...
   println("ShredQuery5Spark"+sf+","+Config.datapath+","+end0+","+spark.sparkContext.applicationId)
 }
}
