...
object Query4SparkOpt {
 def main(args: Array[String]){
   val sf = Config.datapath.split("/").last
   val conf = new SparkConf().setMaster(Config.master).setAppName("Query4SparkOpt"+sf)
...
val x118 = x96.map{ case ((x103, x104), x105) =>
   ({val x107 = (x103.c_name,x104.o_orderdate,x105.p_name) 
x107}, x105.l_qty)
}.reduceByKey(_ + _) 
val x127 = x118
...
x127.count
   ...
   println("Query4SparkOpt"+sf+","+Config.datapath+","+end0)
 }
}
