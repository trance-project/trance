...
case class Record1347(p__F: Part, S__F: Int, PS__F: Int, uniqueId: Long) extends CaseClassRecord
case class Record1348(p__F: Part, C__F: Int, L__F: Int, O__F: Int, uniqueId: Long) extends CaseClassRecord
...
case class Record1441(lbl: Q5Flat, uniqueId: Long) extends CaseClassRecord
case class Input_Query5__D(p_name: String, suppliers: Int, customers: Int, uniqueId: Long) extends CaseClassRecord
case class Record1446(p_name: String, cnt: Int, uniqueId: Long) extends CaseClassRecord
case class Record1447(_1: Q5Flat, _2: Iterable[Record1446], uniqueId: Long) extends CaseClassRecord
...
Query5__D_2customers_1.count
   ...
   def f() {
     val x1374 = Q5Flat(Query5__F, newId) 
val x1375 = Record1441(x1374, newId) 
val x1376 = List(x1375) 
val M_ctx1 = x1376.head
val x1379 = M_ctx1 
val x1380 = Query5__D_1 
val x1382 = x1380 
val x1385 = x1382.map{ case c => (x1379, c) } 
val x1387 = Query5__D_2customers_1 
val x1390 = x1387 
val x1395 = { val out1 = x1385.map{ case (a, null) => (null, (a, null)); case (x1391, x1392) => ({val x1394 = x1392.customers 
x1394}, (x1391, x1392)) }
  val out2 = x1390.flatMap(x1393 => x1393._2.map{case v2 => (x1393._1, v2)})
  out1.outerLookup(out2)
} 
val x1396 = Query5__D_2suppliers_1 
val x1399 = x1396 
val x1407 = { val out1 = x1395.map{ case (a, null) => (null, (a, null)); case ((x1400, x1401), x1402) => (({val x1404 = x1401.suppliers 
x1404}, {val x1406 = x1402.c_nationkey 
x1406}), ((x1400, x1401), x1402)) }
  val out2 = x1399.flatMap(v2 => v2._2.map{case x1403 => ((v2._1, {val x1405 = x1403.s_nationkey 
x1405}), x1403)})
  out1.outerLookup(out2)
} 
val x1415 = x1407.map{ case (((x1408, x1409), x1410), x1411) => val x1414 = (x1411) 
x1414 match {
   case (null) => ({val x1412 = (x1408,x1409,x1410) 
x1412}, 0)
   case x1413 => ({val x1412 = (x1408,x1409,x1410) 
x1412}, {1})
 }
}.reduceByKey(_ + _) 
val x1424 = x1415.map{ case ((x1416, x1417, x1418), x1419) => val x1423 = (x1418) 
x1423 match {
   case x1421 if {val x1422 = x1419 == 0 
x1422} => ({val x1420 = (x1416,x1417) 
x1420}, {1})
   case x1421 => ({val x1420 = (x1416,x1417) 
x1420}, 0)
 }
}.reduceByKey(_ + _) 
val x1433 = x1424.map{ case ((x1425, x1426), x1427) => val x1432 = (x1426,x1427) 
x1432 match {
   case (_,null) => ({val x1428 = (x1425) 
x1428}, null) 
   case x1431 => ({val x1428 = (x1425) 
x1428}, {val x1429 = x1426.p_name 
val x1430 = Record1446(x1429, x1427, newId) 
x1430})
 }
}.groupByLabel() 
val x1438 = x1433.map{ case (x1434, x1435) => 
   val x1436 = x1434.lbl 
val x1437 = Record1447(x1436, x1435, newId) 
x1437 
} 
val M_flat1 = x1438
//println("M_flat1")
