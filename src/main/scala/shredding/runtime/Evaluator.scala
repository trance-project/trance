...
//trait ShredNRCEvaluator extends NRCEvaluator with ScalaRuntime with ShredNRCPrinter {
//  this: ShredNRC =>
//
...
//      ROutLabel(as.map(a => a.varDef -> ctx(a.varDef)).toMap)
//    case Lookup(l, d) =>
//println("LOOKUP.l: " + quote(l))
//println("LOOKUP.d: " + quote(d))
//
//      val rl = evalLabel(l, ctx)
//println("LOOKUP.eval(l): " + rl)
//
//      evalBagDict(d, ctx)(evalLabel(l, ctx))
//    case EmptyDict => REmptyDict
//    case BagDict(l, flat, dict) =>
//println("MILOS BagDict.l: " + quote(l))
//println("MILOS BagDict.flat: " + quote(flat))
//println("MILOS BagDict.dict: " + quote(dict))
//      ROutBagDict(c => evalBag(flat, c), flat.tp, evalTupleDict(dict, ctx))
//    case TupleDict(fs) =>
//      RTupleDict(fs.map(f => f._1 -> eval(f._2, ctx).asInstanceOf[RTupleDictAttribute]))
//    case BagDictProject(t, f) =>
...
//}
//
//trait LinearizedNRCEvaluator extends ShredNRCEvaluator {
//  this: LinearizedNRC =>
//
//  override def eval(e: Expr, ctx: Context): Any = e match {
//    case Named(v, e1) =>
