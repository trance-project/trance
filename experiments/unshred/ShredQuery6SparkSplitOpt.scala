...
def f = {
    ...
    suppliers__D_1_L.cache
    suppliers__D_1_H.cache
    ...
    val top = m__D_1.map(c => c.suppliers -> (c.c_name, c.c_nationkey))
    val result = suppliers__D_1_L.rightLookup(suppliers__D_1_H, top, hkeys4).map{
      case ((cname, nk), supps) => RecordSCR(cname, nk, supps)
    }
