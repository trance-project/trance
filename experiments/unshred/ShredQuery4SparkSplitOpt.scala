...
object ShredQuery4SparkSplitOpt {
 def main(args: Array[String]){
   val sf = Config.datapath.split("/").last
   val conf = new SparkConf().setMaster(Config.master).setAppName("ShredQuery4SparkSplitOpt"+sf)
...
//o_parts__D_1.collect.foreach(println(_))
o_parts__D_1_L.cache
o_parts__D_1_H.cache
spark.sparkContext.runJob(o_parts__D_1_L, (iter: Iterator[_]) => {})
spark.sparkContext.runJob(o_parts__D_1_H, (iter: Iterator[_]) => {})
var end0 = System.currentTimeMillis() - start0
println("ShredQuery4SparkSplitOpt,"+sf+","+Config.datapath+","+end0+",query,"+spark.sparkContext.applicationId)

var start1 = System.currentTimeMillis()
val c_orders__D_1 = query1__D_2c_orders_1_L.unionPartitions(query1__D_2c_orders_1_H).flatMap{
  case (lbl, bag) => bag.map(o => (o.o_parts, (lbl, o.o_orderdate)))
}
val c_orders = o_parts__D_1_L.rightLookup(o_parts__D_1_H, c_orders__D_1, hkeys4).map{
  case ((lbl, date), parts) => lbl -> (date, parts)
}
val result = query1__D_1_L.map(c => c.c_orders -> c.c_name).cogroup(c_orders).flatMap{
  case (_, (cnames, dates)) => cnames.map(c => c -> dates)
}
spark.sparkContext.runJob(result, (iter: Iterator[_]) => {})
var end1 = System.currentTimeMillis() - start1
println("ShredQuery4SparkSplitOpt,"+sf+","+Config.datapath+","+end1+",unshredding,"+spark.sparkContext.applicationId)
