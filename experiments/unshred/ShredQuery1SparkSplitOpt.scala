...
c_orders__D_1_L.cache
c_orders__D_1_H.cache
...
val c_orders__D_1 = c_orders__D_1_L.unionPartitions(c_orders__D_1_H, false).flatMap{
  case (lbl, bag) => bag.map(o => (o.o_parts, (lbl, o.o_orderdate)))
}
val c_orders = o_parts__D_1_L.rightLookup(o_parts__D_1_H, c_orders__D_1, hkeys2).map{
  case ((lbl, date), parts) => lbl -> (date, parts)
}
val result = M__D_1.map(c => c.c_orders -> c.c_name).cogroup(c_orders).flatMap{
