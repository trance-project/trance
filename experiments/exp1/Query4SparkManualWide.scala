...
object Query4SparkManualWide {
  def main(args: Array[String]){
    val sf = Config.datapath.split("/").last
    val conf = new SparkConf().setMaster(Config.master).setAppName("Query4SparkManualWide"+sf)
    ...
    val result = cop.zipWithIndex.flatMap{
      case (ctup, id1) => if (ctup.corders.isEmpty) List((-1, (id1, ctup.c, null, null, null)))
        ...
        case (otup, id2) => if (otup.oparts.isEmpty) List((-1, (id1, ctup.c, id2, otup.o, null)))
            ...
            case ptup => ptup.p.p_partkey -> (id1, ctup.c, id2, otup.o, ptup) 
        }
      }
    }.outerJoinSkew(P.map(p => p.p_partkey -> Record318(p.p_retailprice, p.p_name))).map{
      case ((id1, cust, id2, order, part), Some(pinfo)) =>
        (id1, cust, id2, order, part, pinfo.p_name) -> part.l_qty*pinfo.p_retailprice
      case ((id1, cust, id2, order, part), _) => (id1, cust, id2, order, part, null) -> 0.0
    }.reduceByKey(_ + _).map{
    ...
    spark.sparkContext.runJob(result,  (iter: Iterator[_]) => {})
	  ...
	  println("Query4SparkManualWide,"+sf+","+Config.datapath+","+end0+",query,"+spark.sparkContext.applicationId)
