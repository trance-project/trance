...
object ShredQuery4SparkOptProj2 {
 def main(args: Array[String]){
   val sf = Config.datapath.split("/").last
   val conf = new SparkConf().setMaster(Config.master).setAppName("ShredQuery4SparkOptProj2"+sf)
...
println("ShredQuery4SparkOptProj2,"+sf+","+Config.datapath+","+end0+",query,"+spark.sparkContext.applicationId)

var start1 = System.currentTimeMillis()
/**val x342 = c_orders__D_1.mapPartitions(
    it => it.flatMap{ case (lbl, bag) => bag.map(o => (o.o_parts, (lbl, o)))}, false
...
println("ShredQuery4SparkOptProj2,"+sf+","+Config.datapath+","+end1+",unshredding,"+spark.sparkContext.applicationId)
