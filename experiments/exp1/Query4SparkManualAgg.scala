...
object Query4SparkManualAgg {
  def main(args: Array[String]){
    val sf = Config.datapath.split("/").last
    val conf = new SparkConf().setMaster(Config.master).setAppName("Query4SparkManualAgg"+sf)
      ...
      case (ctup, id1) => if (ctup.corders.isEmpty) List((-1, (id1, ctup.c.c_name, null, null, 0.0)))
        ...
        case (otup, id2) => if (otup.oparts.isEmpty) List((-1, (id1, ctup.c.c_name, id2, otup.o.o_orderdate, 0.0)))
            ...
            (acc, p) => {acc(p.p.p_partkey) += p.l_qty; acc}).map{ case (pk, tot) => 
              pk -> (id1, ctup.c.c_name, id2, otup.o.o_orderdate, tot) }
        }
