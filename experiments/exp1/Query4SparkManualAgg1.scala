...
case class RecordOP(o: Orders, oparts: Iterable[Record373])
case class RecordCOP(c: Customer, corders: Iterable[RecordOP])
object Query4SparkManualAgg1 {
  def main(args: Array[String]){
    val sf = Config.datapath.split("/").last
    val conf = new SparkConf().setMaster(Config.master).setAppName("Query4SparkManualAgg1"+sf)
    ...
    spark.sparkContext.runJob(result,  (iter: Iterator[_]) => {})
	  ...
	  println("Query4SparkManualAgg1,"+sf+","+Config.datapath+","+end0+",query,"+spark.sparkContext.applicationId)
      ...
      c =>
        if (c.corders.isEmpty) List((c.c.c_name, null, null, null))
          ...
          o => 
            ...
            else o.oparts.map(p => (c.c.c_name, o.o.o_orderdate, p.p_name, p._2))
         }
