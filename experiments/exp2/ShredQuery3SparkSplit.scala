...
object ShredQuery3SparkSplit {
 def main(args: Array[String]){
   val sf = Config.datapath.split("/").last
   val conf = new SparkConf().setMaster(Config.master).setAppName("ShredQuery3SparkSplit"+sf)
...
val x222_L = query1__D_2c_orders_1_L.flatMap{ 
  case (lbl, bag) => bag.map(b => (b.o_parts, lbl)) 
}

val x222_H = query1__D_2c_orders_1_H.flatMap{ 
  case (lbl, bag) => bag.map(b => (b.o_parts, lbl)) 
}

val (x227_L, x227_H, hkeys4) = x226_L.joinSplit(x226_H, x222_L.unionPartitions(x222_H))
...
val (x229_L, x229_H, hkeys5) = x228_L.joinSplit(x228_H, x208)

val x230_L = x229_L.map{
  case ((pname, tot), cname) => (cname, pname) -> tot
}

val x230_H = x229_H.map{
  case ((pname, tot), cname) => (cname, pname) -> tot
}

val (m__D_1_L, m__D_1_H) = x230_L.reduceBySplit(x230_H, _+_)
...
println("ShredQuery3SparkSplit,"+sf+","+Config.datapath+","+end0+",query,"+spark.sparkContext.applicationId)
println("ShredQuery3SparkSplit,"+sf+","+Config.datapath+",0,unshredding,"+spark.sparkContext.applicationId)
