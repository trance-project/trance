...
object Query3SparkManualAggUnnestSkew {
  def main(args: Array[String]){
    val sf = Config.datapath.split("/").last
    val conf = new SparkConf().setMaster(Config.master).setAppName("Query3SparkManualAggUnnestSkew"+sf)
    ...
    val result = cop.zipWithIndex.flatMap{
      case (ctup, id1) => if (ctup.c_orders.isEmpty) Nil
        else ctup.c_orders.zipWithIndex.flatMap{
         case (otup, id2) => if (otup.o_parts.isEmpty) Nil         
    ...
    var end0 = System.currentTimeMillis() - start0
	  println("Query3SparkManualAggUnnestSkew"+sf+","+Config.datapath+","+end0+",query,"+spark.sparkContext.applicationId)
