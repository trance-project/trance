...
object Query6SparkManualAgg {
 def main(args: Array[String]){
   val sf = Config.datapath.split("/").last
   val conf = new SparkConf().setMaster(Config.master).setAppName("Query6SparkManualAgg"+sf)
  ...
  val result = spcFlat.cogroup(custs).flatMap{
    case (_, (suppliers, cnames)) => cnames.map(c => RecordCS(c, suppliers))
  }
