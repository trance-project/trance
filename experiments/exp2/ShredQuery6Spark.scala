...
def f = {
    ...
    }, true).cogroup(query5__D_1.map(s => 
      s.customers -> RecordS(s.s_name, s.s_nationkey))).flatMap{
      case (_, (custs, supps)) => custs.map{c => c.c_custkey -> supps}
    }.cogroup(suppliers_ctx1.map(l => l  -> l), new HashPartitioner(400)).mapPartitions(
      it => it.map{case (lbl, (supps, _)) => lbl -> supps.flatten }, true)
    ...
    println("ShredQuery6Spark"+sf+","+Config.datapath+","+end+",unshredding,"+spark.sparkContext.applicationId)
