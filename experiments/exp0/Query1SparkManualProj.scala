...
object Query1SparkManualProj {
  def main(args: Array[String]){
    val sf = Config.datapath.split("/").last
    val conf = new SparkConf().setMaster(Config.master).setAppName("Query1SparkManualProj"+sf)
    ...
    val result = colp.map{
      case ((c, id), orders) => Record233(c.c_name, orders)
    }
    spark.sparkContext.runJob(result, (iter: Iterator[_]) => {})
    var end0 = System.currentTimeMillis() - start0
    println("Query1SparkManualProj"+sf+","+Config.datapath+","+end0+",query,"+spark.sparkContext.applicationId)
