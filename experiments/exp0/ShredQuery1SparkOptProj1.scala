...
object ShredQuery1SparkOptProj1 {
 def main(args: Array[String]){
   val sf = Config.datapath.split("/").last
   val conf = new SparkConf().setMaster(Config.master).setAppName("ShredQuery1SparkOptProj1"+sf)
...
println("ShredQuery1SparkOptProj1,"+sf+","+Config.datapath+","+end0+",query,"+spark.sparkContext.applicationId)
...
println("ShredQuery1SparkOptProj1,"+sf+","+Config.datapath+","+end+",total,"+spark.sparkContext.applicationId)
println("ShredQuery1SparkOptProj1,"+sf+","+Config.datapath+","+end1+",unshredding,"+spark.sparkContext.applicationId)
...
/**result.flatMap{ case (cname, orders) =>
  if (orders.isEmpty) List((cname.c_name, null, null, null))
    ...
    else parts.map(p => (cname.c_name, date.o_orderdate, p.p_name, p.l_qty)) } }.collect.foreach(println(_))**/
}
f
 }
}
