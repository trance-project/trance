...
object Query1SparkManualAgg {
  def main(args: Array[String]){
    val sf = Config.datapath.split("/").last
    val conf = new SparkConf().setMaster(Config.master).setAppName("Query1SparkManualAgg"+sf)
    ...
    println("Query1SparkManualAgg"+sf+","+Config.datapath+","+end0+",query,"+spark.sparkContext.applicationId)
