...
object ShredQuery4SparkSplitOptFilter {
 def main(args: Array[String]){
   val sf = Config.datapath.split("/").last
   val conf = new SparkConf().setMaster(Config.master).setAppName("ShredQuery4SparkSplitOptFilter"+sf)
...
val c_orders__D_1_L = query1__D_2c_orders_1_L.mapPartitions(
    it => it.map{ case (lbl, bag) => (lbl, bag.filter(o => o.o_priority.contains("HIGH")))})

val c_orders__D_1_H = query1__D_2c_orders_1_H.mapPartitions(
    it => it.map{ case (lbl, bag) => (lbl, bag.filter(o => o.o_priority.contains("HIGH")))})
...
println("ShredQuery4SparkSplitOptFilter,"+sf+","+Config.datapath+","+end0+",query,"+spark.sparkContext.applicationId)
...
println("ShredQuery4SparkSplitOptFilter,"+sf+","+Config.datapath+","+end1+",unshredding,"+spark.sparkContext.applicationId)
