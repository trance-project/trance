...
val x261 = O__D_1
val x264 = x261.flatMap{ case x263 => if (x263.o_priority.contains("HIGH")) List((x263.o_custkey, x263)) else Nil }
val (x267_L, x267_H, hkeys1) = x264.joinDomainSplit(x255)

// don't even need this mapping...
val x277_L = x267_L.mapPartitions(it =>
  it.map{ case (lbl, x269) => 
   (lbl, {val x271 = x269.o_orderdate 
...
val (x304_L, x304_H, hkeys2) = (x303_L.unionPartitions(x303_H)).joinDomainSplit(x285_L.unionPartitions(x285_H))

val x315_L = x304_L.mapPartitions(it =>
  it.map{ case (lbl, x308) => 
  (lbl, {val x310 = x308.p_partkey 
val x311 = x308.l_qty 
