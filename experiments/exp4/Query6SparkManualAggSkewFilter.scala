...
object Query6SparkManualAggSkewFilter {
 def main(args: Array[String]){
   val sf = Config.datapath.split("/").last
   val conf = new SparkConf().setMaster(Config.master).setAppName("Query6SparkManualAggSkewFilter"+sf)
  ...
  val custs = C.zipWithIndex.flatMap{ case (c, id) => 
    if (c.c_nationkey < 13) List((c.c_custkey, RecordCN(c.c_name, c.c_nationkey))) else Nil}
