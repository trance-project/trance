...
object ShredQuery6SparkSplitOptFilter {
 def main(args: Array[String]){
   val sf = Config.datapath.split("/").last
   val conf = new SparkConf().setMaster(Config.master).setAppName("ShredQuery6SparkSplitOptFilter"+sf)
...
def f = {
    ...
    println("ShredQuery6SparkSplitOptFilter,"+sf+","+Config.datapath+","+end0+",query,"+spark.sparkContext.applicationId)
    var start = System.currentTimeMillis()
    /**val suppliers__D_1 = suppliers__D_1_L.unionPartitions(suppliers__D_1_H, false)
    ...
    println("ShredQuery6SparkSplitOptFilter"+sf+","+Config.datapath+","+end+",unshredding,"+spark.sparkContext.applicationId)
    /**result.flatMap{
