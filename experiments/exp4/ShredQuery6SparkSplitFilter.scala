...
object ShredQuery6SparkSplitFilter {
 def main(args: Array[String]){
   val sf = Config.datapath.split("/").last
   val conf = new SparkConf().setMaster(Config.master).setAppName("ShredQuery6SparkSplitFilter"+sf)
...
def f = {
    ...
    val x169 = C__D_1.flatMap{ case c => 
      if (c.c_nationkey < 13) List(RecordCN(c.c_name, c.c_nationkey, c.c_custkey)) else Nil }
    val m__D_1 = x169

    // m__D_1.cache
    val suppliers_ctx1 = m__D_1.createDomain(l => l.suppliers)
    ...
    val (x152_L, x152_H) = x151_L.cogroupSplit(x151_H, topd, hkeys3)

    val x153_L = x152_L.map{
      case (c, supps) => c.c_custkey -> supps}

    val x153_H = x152_H.map{
      case (c, supps) => c.c_custkey -> supps}

    val (x154_L, x154_H, hkeys4) = x153_L.unionPartitions(x153_H).cogroupDomainSplit(suppliers_ctx1)
    ...
    println("ShredQuery6SparkSplitFilter,"+sf+","+Config.datapath+","+end0+",query,"+spark.sparkContext.applicationId)
    ...
    println("ShredQuery6SparkSplitFilter"+sf+","+Config.datapath+","+end+",unshredding,"+spark.sparkContext.applicationId)
