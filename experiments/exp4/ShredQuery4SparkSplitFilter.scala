...
case class Record333(o_orderdate: String, o_priority: String, o_parts: Int)
...
object ShredQuery4SparkSplitFilter {
 def main(args: Array[String]){
   val sf = Config.datapath.split("/").last
   val conf = new SparkConf().setMaster(Config.master).setAppName("ShredQuery4SparkSplitFilter"+sf)
...
val x273 = (x272) 
val x274 = Record333(x271, x269.o_priority, x273) 
x274})
}, true)
...
val (x276_L, x276_H) = query1__D_2c_orders_1_L.lookupSplit(query1__D_2c_orders_1_H, 
  c_orders_ctx1, (l: Int) => l, hkeys1)

val x275_L = x276_L.mapPartitions(
    it => it.map{ case (lbl, bag) => (lbl, bag.filter(o => o.o_priority.contains("HIGH")))})

val x275_H = x276_H.mapPartitions(
    it => it.map{ case (lbl, bag) => (lbl, bag.filter(o => o.o_priority.contains("HIGH")))})

val c_orders__D_1_L = x275_L
val c_orders__D_1_H = x275_H
...
println("ShredQuery4SparkSplitFilter,"+sf+","+Config.datapath+","+end0+",query,"+spark.sparkContext.applicationId)

var start1 = System.currentTimeMillis()
/**val x342 = c_orders__D_1_L.unionPartitions(c_orders__D_1_H, false).flatMap{
...
println("ShredQuery4SparkSplitFilter,"+sf+","+Config.datapath+","+end1+",unshredding,"+spark.sparkContext.applicationId)
