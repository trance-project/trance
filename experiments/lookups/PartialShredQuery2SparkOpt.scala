...
object PartialShredQuery2SparkOpt {
 def main(args: Array[String]){
   val sf = Config.datapath.split("/").last
   val conf = new SparkConf().setMaster(Config.master).setAppName("PartialShredQuery2SparkOpt"+sf)
...
val partialLevel2 = Query1__D_2c_orders_1.mapPartitions{
  it => it.flatMap{ case (lbl, bag) => bag.map(b => (b.o_parts, (lbl, b.o_orderdate)))}
}.cogroup(Query1__D_2c_orders_2o_parts_1).mapPartitions(
  it => it.flatMap{
...
val x383 = partialLevel2
val x384 = x383.mapPartitions{
  it => it.flatMap{ case (lbl, bag) => bag.flatMap{ case (date, parts) => parts.map(p => (lbl, date, p.p_partkey) -> p.l_qty) }}
}.reduceByKey(_+_).map{
  case ((lbl, date, pk), total) => Record412(lbl) -> (date, pk, total)
}.groupByLabel()
val totals__D_1 = x384
spark.sparkContext.runJob(totals__D_1, (iter: Iterator[_]) => {})
var end0 = System.currentTimeMillis() - start0
println("PartialShredQuery2SparkOpt,"+sf+","+Config.datapath+","+end0+",query,"+spark.sparkContext.applicationId)
...
println("PartialShredQuery2SparkOpt,"+sf+","+Config.datapath+","+end+",total,"+spark.sparkContext.applicationId)
println("PartialShredQuery2SparkOpt,"+sf+","+Config.datapath+","+end1+",unshredding,"+spark.sparkContext.applicationId)
