...
val x217 = Query1__D_2c_orders_2o_parts_1
val x218 = x217.mapPartitions(
  it => it.flatMap{ case (lbl, bag) => bag.map(p => p.p_partkey -> (lbl, p.l_qty))}, true)
val x221 = P__D_1
val x222 = x218.joinSkew(x221, (p: PartProj4) => p.p_partkey).map{
  case ((lbl, qty), p2) => (lbl, p2.p_name) -> qty*p2.p_retailprice
}.reduceByKey(_+_).map{ 
...
val x241 = x207.map(c => c.c_orders -> c.c_name).joinSkew(x209).mapPartitions(
  ...
  case (cname, (pname, tot)) => (cname, pname) -> tot
}.reduceByKey(_+_).map{
  case ((cname, pname), tot) => (cname, pname, tot)
}
val M__D_1 = x241
