...
object PartialShredQuery2Spark {
 def main(args: Array[String]){
   val sf = Config.datapath.split("/").last
   val conf = new SparkConf().setMaster(Config.master).setAppName("PartialShredQuery2Spark"+sf)
...
val x384 = x383.lookupSkew(x382, (l: Record412) => l.c2__Fc_orders).flatMap{
  case (lbl, bag) => bag.flatMap{ case (date, parts) => parts.map(p => (lbl, date, p.p_partkey) -> p.l_qty) }
}.reduceByKey(_+_).map{
...
println("PartialShredQuery2Spark,"+sf+","+Config.datapath+","+end0+",query,"+spark.sparkContext.applicationId)
...
println("PartialShredQuery2Spark,"+sf+","+Config.datapath+","+end+",total,"+spark.sparkContext.applicationId)
println("PartialShredQuery2Spark,"+sf+","+Config.datapath+","+end1+",unshredding,"+spark.sparkContext.applicationId)
