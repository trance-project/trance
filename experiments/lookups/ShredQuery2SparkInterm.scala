...
object ShredQuery2SparkInterm {
 def main(args: Array[String]){
   val sf = Config.datapath.split("/").last
   val conf = new SparkConf().setMaster(Config.master).setAppName("ShredQuery2SparkInterm"+sf)
...
val x386 = x384.join(x385).mapPartitions{
  it => it.flatMap{ case (_, ((lbl, date), parts)) => 
    parts.map(p => (lbl, date, p._1) -> p._2) }
}.reduceByKey(_+_).map{
...
println("ShredQuery2SparkInterm,"+sf+","+Config.datapath+","+end0+",query,"+spark.sparkContext.applicationId)
...
println("ShredQuery2SparkInterm,"+sf+","+Config.datapath+","+end+",total,"+spark.sparkContext.applicationId)
println("ShredQuery2SparkInterm,"+sf+","+Config.datapath+","+end1+",unshredding,"+spark.sparkContext.applicationId)
