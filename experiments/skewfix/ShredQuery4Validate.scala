...
object ShredQuery4Validate {
 def main(args: Array[String]){
   val sf = Config.datapath.split("/").last
   val conf = new SparkConf().setMaster(Config.master).setAppName("ShredQuery4Validate"+sf)
...
out1.cogroup(o_parts__D_1).flatMap{
 case (_, (left, x349)) => left.map{ case (x347, x348) => ((x347, x348), x349.flatMap{ m => m.map(t => (t.p_name, t._2)) }) }}
}
