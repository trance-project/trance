...
object ShredQuery1SparkBroadcast {
 def main(args: Array[String]){
   val sf = Config.datapath.split("/").last
   val conf = new SparkConf().setMaster(Config.master).setAppName("ShredQuery1SparkBroadcast"+sf)
...
x102 }) 
val domain1 = x103.sparkContext.broadcast(x97.map{ case x104 => ({val x106 = x104.lbl 
val x107 = x106.c__Fc_custkey 
x107}, x104) }.collect.toMap)
  val x109_out2 = x103.map{ case x105 => ({val x108 = x105.o_custkey 
x108}, x105) }
val x124 = x109_out2.flatMap{ case (key, value) =>
...
println("ShredQuery1SparkBroadcast,"+sf+","+Config.datapath+","+end0+",query,"+spark.sparkContext.applicationId)
...
println("ShredQuery1SparkBroadcast,"+sf+","+Config.datapath+","+end1+",unshredding,"+spark.sparkContext.applicationId)
