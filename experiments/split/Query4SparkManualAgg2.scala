...
case class PartExtend(p: RecordLP, p_name: String, total: Double)
case class RecordOP(orderdate: String, oparts: Iterable[Record373])
case class RecordCOP(c: Customer, corders: Iterable[RecordOP])
object Query4SparkManualAgg2 {
  def main(args: Array[String]){
    val sf = Config.datapath.split("/").last
    val conf = new SparkConf().setMaster(Config.master).setAppName("Query4SparkManualAgg2"+sf)
    ...
    val result = cop.zipWithIndex.flatMap{
      case (ctup, id1) => if (ctup.corders.isEmpty) List((-1, (id1, ctup.c, null, null, null)))
        ...
        case (otup, id2) => if (otup.oparts.isEmpty) List((-1, (id1, ctup.c, id2, otup.o.o_orderdate, null)))
          else otup.oparts.foldLeft(HashMap.empty[Int, Double].withDefaultValue(0.0))(
            (acc, p) => {acc(p.p.p_partkey) += p.l_qty; acc}).map{ case (pk, tot) =>
              pk -> (id1, ctup.c, id2, otup.o.o_orderdate, tot) 
          }
       }
    }.leftOuterJoin(P.map(p => p.p_partkey -> Record318(p.p_retailprice, p.p_name))).map{
      case (_, ((id1, cust, id2, order, qty:Double), Some(pinfo))) =>
    ...
    spark.sparkContext.runJob(result,  (iter: Iterator[_]) => {})
	  ...
	  println("Query4SparkManualAgg2,"+sf+","+Config.datapath+","+end0+",query,"+spark.sparkContext.applicationId)
    ...
    /**result.flatMap{
      c =>
        if (c.corders.isEmpty) List((c.c.c_name, null, null, null))
          ...
          o => 
            if (o.oparts.isEmpty) List((c.c.c_name, o.orderdate, null, null))
            else o.oparts.map(p => (c.c.c_name, o.orderdate, p.p_name, p._2))
         }
