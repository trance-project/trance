...
object Query4SparkManualProj1 {
  def main(args: Array[String]){
    val sf = Config.datapath.split("/").last
    val conf = new SparkConf().setMaster(Config.master).setAppName("Query4SparkManualProj1"+sf)
    ...
    val result = cop.zipWithIndex.flatMap{
      case (ctup, id1) => if (ctup.corders.isEmpty) List((-1, (id1, ctup.c, null, null, null)))
        ...
        case (otup, id2) => if (otup.oparts.isEmpty) List((-1, (id1, ctup.c, id2, otup.o, null)))
            ...
            case ptup => ptup.p.p_partkey -> (id1, ctup.c, id2, otup.o, ptup.l_qty) 
        }
      }
    ...
    spark.sparkContext.runJob(result,  (iter: Iterator[_]) => {})
	  ...
	  println("Query4SparkManualProj1,"+sf+","+Config.datapath+","+end0+",query,"+spark.sparkContext.applicationId)
