...
object Query2SparkManualPush {
  def main(args: Array[String]){
    val sf = Config.datapath.split("/").last
    val conf = new SparkConf().setMaster(Config.master).setAppName("Query2SparkManualPush"+sf)
    ...
    val result = c.zipWithIndex.flatMap{
      case (ctup, id1) => if (ctup.c_orders.isEmpty) List(((id1, ctup.c_name, null, null),0.0))
        ...
        case (otup, id2) => if (otup.o_parts.isEmpty) List(((id1, ctup.c_name, otup.o_orderdate, null),0.0))
          else otup.o_parts.foldLeft(HashMap.empty[(Long, String, String, Int), Double].withDefaultValue(0))(
    ...
    var end0 = System.currentTimeMillis() - start0
	  println("Query2SparkManualPush"+sf+","+Config.datapath+","+end0+",query,"+spark.sparkContext.applicationId)
