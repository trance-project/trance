...
object Query3SparkManualWide {
  def main(args: Array[String]){
    ...
    val CustomerOrders = O.map(o => o.o_orderkey -> o).cogroup(OrderParts).flatMap{
      case (_, (orders, parts)) => orders.map( order => order.o_custkey -> RecordOLP(order, parts.toList)) 
    }
    ...
    val result = c.zipWithIndex.flatMap{
      case (ctup, id1) => if (ctup.corders.isEmpty) List((-1, (id1, ctup, null)))
      ...
      case (k, ((id1, cname, lp), p)) => (cname, lp.p) -> lp.l.l_quantity*p.p_retailprice
    }.reduceByKey(_+_).map{
      case ((cname, pname), total) => (cname, pname, total)
    }
