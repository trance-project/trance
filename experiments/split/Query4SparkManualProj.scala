...
object Query4SparkManualProj {
  def main(args: Array[String]){
    val sf = Config.datapath.split("/").last
    val conf = new SparkConf().setMaster(Config.master).setAppName("Query4SparkManualProj"+sf)
    ...
    val result = cop.zipWithIndex.flatMap{
      case (ctup, id1) => if (ctup.corders.isEmpty) List((-1, (id1, ctup.c.c_name, null, null, 0.0)))
        else ctup.corders.zipWithIndex.flatMap{
        case (otup, id2) => if (otup.oparts.isEmpty) List((-1, (id1, ctup.c.c_name, id2, otup.o.o_orderdate, 0.0)))
          else otup.oparts.map{
            case ptup => (ptup.p.p_partkey, (id1, ctup.c.c_name, id2, otup.o.o_orderdate, ptup.l_qty)) 
        }
      }
    ...
    spark.sparkContext.runJob(result,  (iter: Iterator[_]) => {})
	  ...
	  println("Query4SparkManualProj,"+sf+","+Config.datapath+","+end0+",query,"+spark.sparkContext.applicationId)
