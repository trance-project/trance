...
object Query1SparkManualAgg2 {
  def main(args: Array[String]){
    val sf = Config.datapath.split("/").last
    val conf = new SparkConf().setMaster(Config.master).setAppName("Query1SparkManualAgg2"+sf)
    ...
    val OrderParts = lpj.map{ case (k, (l, p)) => l.l_orderkey -> RecordLP(p.p_name, l.l_orderkey, l.l_quantity) }

    val CustomerOrders = O.zipWithIndex.map{ case (o, id) => 
      o.o_orderkey -> (Record173(o.o_orderdate, o.o_orderkey, o.o_custkey), id) }.cogroup(OrderParts).flatMap{
        case (ok, (orders, parts)) => orders.map{ case (o, id) => o.o_custkey -> RecordOLP(o.o_orderdate, parts) }}
    ...
    println("Query1SparkManualAgg2"+sf+","+Config.datapath+","+end0+",query,"+spark.sparkContext.applicationId)
