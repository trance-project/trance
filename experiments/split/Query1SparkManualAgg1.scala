...
object Query1SparkManualAgg1 {
  def main(args: Array[String]){
    val sf = Config.datapath.split("/").last
    val conf = new SparkConf().setMaster(Config.master).setAppName("Query1SparkManualAgg1"+sf)
    ...
    println("Query1SparkManualAgg1"+sf+","+Config.datapath+","+end0+",query,"+spark.sparkContext.applicationId)
    ...
    /**result.flatMap(c => 
      if (c.corders.isEmpty) List((c.c.c_name, null, null, null))
         ...
         else o.oparts.map(p => (c.c.c_name, o.o.o_orderdate, p.p_name, p.l_qty)))).sortBy(_._1).collect.foreach(println(_))**/
