...
object Query4New2SparkManual {
  def main(args: Array[String]){
    val sf = Config.datapath.split("/").last
    val conf = new SparkConf().setMaster(Config.master).setAppName("Query4New2SparkManual"+sf)
    ...
    var start0 = System.currentTimeMillis()
      ...
      case (cname, orders) => orders.map{ case (date, parts) => cname -> date }
    }.join(C.map(c => c.c_name -> c.c_address)).map{
      case (cname, (date, address)) => date -> (cname, address)
    }.groupByKey()
	var end0 = System.currentTimeMillis() - start0
    println("Query4New2SparkManual"+sf+","+Config.datapath+","+end0)
  }
}
