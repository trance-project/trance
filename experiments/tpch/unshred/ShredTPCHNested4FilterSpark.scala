...
object ShredTPCHNested4FilterSpark {
 def main(args: Array[String]){
   val sf = Config.datapath.split("/").last
   val conf = new SparkConf().setMaster(Config.master).setAppName("ShredTPCHNested4FilterSpark"+sf)
...
val cnames = C__D_1.filter(c => c.c_custkey <= 1500000).map(c => c.c_name).collect.toList
val dates = O__D_1.filter(o => o.o_orderkey <= 150000000).map(o => o.o_orderdate).collect.toList
...
val x221 = M_ctx1
//M_ctx1.collect.foreach(println(_))
val x227 = Query1__D_1.filter(c => cnames.contains(c.c_name)).map{ case x222 => 
...
x252 match {
   case (null) => Nil 
   case x251 => if (dates.contains(x245.o_orderdate)) {
     List(({val x246 = (x244) 
x246}, {val x247 = x245.o_orderdate 
val x248 = x245.o_parts 
val x249 = Record315(x248) 
val x250 = Record316(x247, x249) 
x250}))
 }
 else Nil
}}.groupByLabel() 
val x258 = x253.map{ case (x254, x255) => 
   val x256 = x254.lbl 
val x257 = (x256, x255) 
x257 
} 
...
println("ShredTPCHNested4FilterSpark,"+sf+","+Config.datapath+","+end0+",query,"+spark.sparkContext.applicationId)
...
var end = System.currentTimeMillis() - start
   ...
   println("ShredTPCHNested4FilterSpark"+sf+","+Config.datapath+","+end+",total,"+spark.sparkContext.applicationId)
 }
}
