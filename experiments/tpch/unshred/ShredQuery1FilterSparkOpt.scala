...
object ShredQuery1FilterSparkOpt {
 def main(args: Array[String]){
   val sf = Config.datapath.split("/").last
   val conf = new SparkConf().setMaster(Config.master).setAppName("ShredQuery1FilterSparkOpt"+sf)
...
val x158 = C__D_1.filter(_.c_custkey <= 1500000).map{ case x153 => 
...
println("ShredQuery1FilterSparkOpt"+sf+","+Config.datapath+","+end0+",query,"+spark.sparkContext.applicationId)
...
println("ShredQuery1FilterSparkOpt"+sf+","+Config.datapath+","+end1+",unshredding,"+spark.sparkContext.applicationId)

}
var start = System.currentTimeMillis()
f
var end = System.currentTimeMillis() - start
   println("ShredQuery1FilterSparkOpt"+sf+","+Config.datapath+","+end+",total,"+spark.sparkContext.applicationId)
 }
}
