...
case class Record236(o_orderdate: String, o_parts: Iterable[Record183])
case class Record237(c_name: String, c_orders: Iterable[Record236])
...
val x205 = c_orders__D_1 
val x209 = x205.flatMap{ 
 case x206 => {val x207 = x206._2 
x207}.map{ case v2 => (x206._1, v2) }
}
         
val x214 = { val out1 = x209.map{ case (x210, x211) => ({val x213 = x211.o_parts 
x213}, (x210, x211)) }
out1.cogroup(o_parts__D_1).flatMap{
 case (_, (left, x212)) => left.map{ case (x210, x211) => ((x210, x211), x212.flatten) }}
}
         
val x221 = x214.map{ case ((x215, x216), x217) => 
   val x218 = x216.o_orderdate 
val x219 = Record236(x218, x217) 
val x220 = (x215, x219) 
x220 
} 
val newc_orders__D_1 = x221
val x222 = newc_orders__D_1
//newc_orders__D_1.collect.foreach(println(_))
val x224 = M__D_1 
val x228 = { val out1 = x224.map{ case x225 => ({val x227 = x225.c_orders 
x227}, x225) }
out1.cogroup(newc_orders__D_1).flatMap{
 case (_, (left, x226)) => left.map{ case x225 => (x225, x226) }}
}
         
val x233 = x228.map{ case (x229, x230) => 
   val x231 = x229.c_name 
val x232 = Record237(x231, x230) 
x232 
} 
val newM__D_1 = x233
val x234 = newM__D_1
...
println("ShredQuery1FilterSpark,"+sf+","+Config.datapath+","+end1+",unshredding,"+spark.sparkContext.applicationId)
