...
object ShredQuery4Filter2SparkOpt {
 def main(args: Array[String]){
   val sf = Config.datapath.split("/").last
   val conf = new SparkConf().setMaster(Config.master).setAppName("ShredQuery4Filter2SparkOpt"+sf)
...
val M__D_1 = x240
val x241 = M__D_1
val x258 = Query1Extended__D_2c_orders_1.flatMap{
  case (lbl, bag) => bag.flatMap{
    case o => if (o.o_orderkey <= 150000000) List((lbl, o)) else Nil
  }
}
         
val x268 = x258.flatMap{ case (x259, x260) => val x267 = (x260) 
x267 match {
   case (null) => Nil 
...
val x296 = Query1Extended__D_2c_orders_2o_parts_1.flatMapValues(identity)
...
println("ShredQuery4Filter2SparkOpt,"+sf+","+Config.datapath+","+end0+",query,"+spark.sparkContext.applicationId)
...
var end = System.currentTimeMillis() - start
   ...
   println("ShredQuery4Filter2SparkOpt"+sf+","+Config.datapath+","+end+",total,"+spark.sparkContext.applicationId)
 }
}
