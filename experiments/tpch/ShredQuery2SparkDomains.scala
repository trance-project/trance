...
object ShredQuery2SparkDomains {
 def main(args: Array[String]){
   val sf = Config.datapath.split("/").last
   val conf = new SparkConf().setMaster(Config.master).setAppName("ShredQuery2SparkDomains"+sf)
...
val x197 = x189.flatMap{ case (x190, x191) => val x196 = (x191) 
x196 match {
   case (null) => Nil 
   case x195 => List(({val x192 = (x190) 
