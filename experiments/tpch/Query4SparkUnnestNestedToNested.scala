...
object Query4SparkUnnestNestedToNested {
 def main(args: Array[String]){
   val sf = Config.datapath.split("/").last
   val conf = new SparkConf().setMaster(Config.master).setAppName("Query4SparkUnnestNestedToNested"+sf)
...
var end0 = System.currentTimeMillis() - start0 
   println("Query4SparkUnnestNestedToNested"+sf+","+Config.datapath+","+end0+","+spark.sparkContext.applicationId)
 }
}
