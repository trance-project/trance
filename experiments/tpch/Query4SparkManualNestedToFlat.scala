...
object Query4SparkManualNestedToFlat {
  def main(args: Array[String]){
    val sf = Config.datapath.split("/").last
    val conf = new SparkConf().setMaster(Config.master).setAppName("Query4SparkManualNestedToFlat"+sf)
    ...
    val custords = c.flatMap{ case (cname, orders) => orders }.join(lpj)
                    .map{ case (_, (orderdate, (pname, qty))) => 
                      ((orderdate, pname), qty)
                   }.reduceByKey(_ + _)
    custords.count
    var end0 = System.currentTimeMillis() - start0
    println("Query4SparkManualNestedToFlat"+sf+","+Config.datapath+","+end0+","+spark.sparkContext.applicationId)
  }
}
