...
object ShredQuery1Spark2Domains {
 def main(args: Array[String]){
   val sf = Config.datapath.split("/").last
   val conf = new SparkConf().setMaster(Config.master).setAppName("ShredQuery1Spark2Domains"+sf)
...
val x238 = x234.flatMap{ case x235 => x235 match {
   ...
   val x236 = x235._2 
x236 match {
     case x237 => x237//x237.map{ case v2 => (x235, v2) }
  }
 }} 
val x243 = x238.map{ case x240 => 
   val x241 = x240.asInstanceOf[Record282].o_parts 
...
var end0 = System.currentTimeMillis() - start0 
   println("ShredQuery1Spark2Domains"+sf+","+Config.datapath+","+end0+","+spark.sparkContext.applicationId)
 }
}
