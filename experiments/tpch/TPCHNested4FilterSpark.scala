...
object TPCHNested4FilterSpark {
 def main(args: Array[String]){
   val sf = Config.datapath.split("/").last
   val conf = new SparkConf().setMaster(Config.master).setAppName("TPCHNested4FilterSpark"+sf)
...
x149} match {
     ...
     case lst => if (cnames.contains(x148.c_name)) { lst.map{ case x150 => (x148, x150) } } else Nil
  }
 }} 
...
x154} match {
     ...
     case lst => if (dates.contains(x153.o_orderdate)) { lst.map{ case x155 => ((x152, x153), x155) }} else Nil
  }
 }} 
...
//x196.collect.foreach(println(_))
x196.count
}
var start = System.currentTimeMillis()
f
var end = System.currentTimeMillis() - start 
   println("TPCHNested4FilterSpark"+sf+","+Config.datapath+","+end+",total,"+spark.sparkContext.applicationId)
 }
}
