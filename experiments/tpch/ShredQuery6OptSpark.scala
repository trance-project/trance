...
object ShredQuery6OptSpark {
 def main(args: Array[String]){
   val sf = Config.datapath.split("/").last
   val conf = new SparkConf().setMaster(Config.master).setAppName("ShredQuery6OptSpark"+sf)
...
val x117 = x108.map{ case (x109, x110) => 
...
var end0 = System.currentTimeMillis() - start0 
   println("ShredQuery6OptSpark"+sf+","+Config.datapath+","+end0+","+spark.sparkContext.applicationId)
 }
}
